...
/* GSM 03.40 SMS-SUBMIT first-octet bits we keep when matching the MCI
 * shape: TP-RP / TP-UDHI / TP-SRR / TP-MTI. TP-RD and TP-VPF are cleared
 * and any TP-VP field is dropped. */
...
/* Rewrite a phone SMS-SUBMIT so sm-RP-UI looks like the working MCI
 * example (FO 0x01: no reject-duplicates, no validity-period). Concatenated
 * SMS keeps UDHI. Returns new length, or 0 if the TPDU is not a SUBMIT we
 * can parse (caller must send the original). */
...
void sms_iwf_on_gsup_mo_req(int conn_id, const gsup_parsed_t *m)
{
    ...
    /* Irancell's SC (and their Huawei tracer) accept the MCI-style
     * SMS-SUBMIT: FO 0x01, no TP-RD, no TP-VP. Handsets often send 0x15
     * (reject-duplicates + relative VP). Keep UDHI for concatenated SMS. */
    uint8_t ui[256];
    size_t ui_len = sms_mo_plain_submit(m->sm_rp_ui, m->sm_rp_ui_len,
                                       ui, sizeof(ui));
    if (!ui_len) {
        memcpy(ui, m->sm_rp_ui, m->sm_rp_ui_len);
        ui_len = m->sm_rp_ui_len;
    } else if (ui_len != m->sm_rp_ui_len || ui[0] != m->sm_rp_ui[0]) {
        LOGI("sms",
             "[%s] MO-FSM TPDU SUBMIT fo=0x%02x->0x%02x ui_len=%u->%u",
             m->imsi, (unsigned)m->sm_rp_ui[0], (unsigned)ui[0],
             (unsigned)m->sm_rp_ui_len, (unsigned)ui_len);
    }
