...
/* Last osmo-sgsn GSUP TCP conn per IMSI (for HSS-initiated CLR). */
typedef struct {
    char imsi[16];
    int  conn_id;
    UT_hash_handle hh;
} gsup_sgsn_conn_t;
static gsup_sgsn_conn_t *g_sgsn_conn;

static void gsup_track_sgsn_conn(const char *imsi, int conn_id)
{
    if (!imsi || !imsi[0] || conn_id < 0) return;
    gsup_sgsn_conn_t *e = NULL;
    HASH_FIND_STR(g_sgsn_conn, imsi, e);
    if (!e) {
        e = calloc(1, sizeof(*e));
        if (!e) return;
        strncpy(e->imsi, imsi, sizeof(e->imsi) - 1);
        HASH_ADD_STR(g_sgsn_conn, imsi, e);
    }
    e->conn_id = conn_id;
}

static int gsup_conn_for_imsi(const char *imsi)
{
    gsup_sgsn_conn_t *e = NULL;
    if (imsi && imsi[0])
        HASH_FIND_STR(g_sgsn_conn, imsi, e);
    return e ? e->conn_id : -1;
}
...
{
    return gsup_server_send(conn_id, gsup, len);
}
                            ...
                            uint8_t cancel_type)
{
    ...
    int conn_id = gsup_conn_for_imsi(imsi);
                            ...
                            const uint8_t *gsup, size_t len)
{
    ...
    if (req.msg_type == GSUP_MSG_ISD_RES || req.msg_type == GSUP_MSG_ISD_ERR) {
        ...
        LOGI("gsup", "RX ISD_RES imsi=%s conn=%d", req.imsi, conn_id);
    ...
    case GSUP_MSG_SAI_REQ:
        gsup_track_sgsn_conn(req.imsi, conn_id);
...
void gsup_map_proxy_finish_ugl(iwf_runtime_t *rt, map_session_t *s)
{
    if (!rt || !s || !s->gsup_originated) return;
    uint8_t gsup[128];
    int n = gsup_build_ul_res(s->imsi_str,
                              s->msisdn_str[0] ? s->msisdn_str : NULL,
                              s->gsup_cn_domain,
                              gsup, sizeof(gsup));
    if (n > 0) {
        (void)proxy_send_gsup(s->gsup_conn_id, gsup, (size_t)n);
        LOGI("gsup", "TX UL_RES imsi=%s msisdn=%s cn=%s conn=%d",
             s->imsi_str,
             s->msisdn_str[0] ? s->msisdn_str : "(none)",
             s->gsup_cn_domain == GSUP_CN_DOMAIN_CS ? "CS" : "PS",
             s->gsup_conn_id);
...
static void map_to_gsup_ugl(gsup_pending_t *p, const uint8_t *params, size_t plen)
{
    ...
    (void)plen;
    uint8_t gsup[128];
    int gn = gsup_build_ul_res(p->imsi, NULL, 0, gsup, sizeof(gsup));
    if (gn > 0) {
        (void)proxy_send_gsup(p->conn_id, gsup, (size_t)gn);
...
void gsup_map_proxy_shutdown(void)
{
    ...
    gsup_sgsn_conn_t *c, *ctmp;
    HASH_ITER(hh, g_sgsn_conn, c, ctmp) {
        HASH_DEL(g_sgsn_conn, c);
        free(c);
    }
    g_rt = NULL;
}
