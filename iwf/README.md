...
`[smf] ERROR: Unknown RAT Type [1]` (smf/s5c-handler.c) means Open5GS SMF received **RAT Type = UTRAN (1)** and only accepts **EUTRAN (6)** and **WLAN (3)**. SMF returns Cause **MANDATORY_IE_INCORRECT**. **`[iwf] rat_type = eutran`** (or `wlan`) **wins**: S4 RAT stays 6, and a real Gn SAI/CGI is mapped to **TAI+ECGI** (PLMN from the cell, **TAC=LAC**, **ECI=CI/SAC**) so location is not dropped. Dummy LAC `0xfffe` / missing ULI still uses the configured gateway TAC/ECI. Set `rat_type = utran` only if the peer actually accepts UTRAN and should receive a truthful SAI.
...
```bash
curl -sS 'http://127.0.0.1:9090/admin/trace/imsi?imsi=001010'
curl -sS 'http://127.0.0.1:9090/admin/trace/imsi?imsi=001010000000001&match=exact'
curl -sS 'http://127.0.0.1:9090/admin/trace/imsi?imsi=list'
curl -sS 'http://127.0.0.1:9090/admin/trace/imsi?imsi=001010&remove=1'
curl -sS 'http://127.0.0.1:9090/admin/trace/imsi?force=1'
curl -sS 'http://127.0.0.1:9090/admin/trace/imsi?imsi=001010000000001&match=exact&replace=1'
```
...
peer_port     = 3868
origin_host   = iwf.mnc001.mcc001.3gppnetwork.org
origin_realm  = mnc001.mcc001.3gppnetwork.org
dest_host     = pyhss.mnc001.mcc001.3gppnetwork.org
...
| **SIGUSR1** | Sends the same test for IMSI `001010000000003` with **no** UNIX reply (log only). Timeout is controlled like other dialogues (`t_dialogue_ms`, default 10s). |
...
```bash
printf 'sai 001010000000003\n' | socat - UNIX-CONNECT:/tmp/iwf_cmd.sock
printf 'msrn_lookup 990000000123\n' | socat - UNIX-CONNECT:/tmp/iwf_cmd.sock
printf 'msrn_show\n' | socat - UNIX-CONNECT:/tmp/iwf_cmd.sock
```
